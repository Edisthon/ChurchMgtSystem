...
import service.AccountsService; // Added for RMI
import java.rmi.registry.LocateRegistry; // Added for RMI
import java.rmi.registry.Registry; // Added for RMI
import model.Accounts; // Added for RMI return type
...
public class LoginForm extends javax.swing.JFrame {
    ...
    // private String generatedOtp; // No longer needed as OTP is not stored client-side after server sends it via email
    ...
    private AccountsService accountsService; // Added RMI service member


    public LoginForm() {
        // Initialize RMI Service first
        try {
            Registry registry = LocateRegistry.getRegistry("127.0.0.1", 6000);
            this.accountsService = (AccountsService) registry.lookup("account"); // Assuming "account" is the RMI name
            System.out.println("AccountsService RMI object found for LoginForm.");
        } catch (Exception e) {
            e.printStackTrace();
            this.accountsService = null;
            // Error will be shown when user tries to use OTP features
        }
    ...
    private void sendOtpButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (accountsService == null) {
            JOptionPane.showMessageDialog(this, "Account service not available. Please try again later.", "Service Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        try {
            String serverResponse = accountsService.requestOtp(user);
            // Display the server's response message (e.g., "OTP has been sent..." or an error)
            JOptionPane.showMessageDialog(this, serverResponse, "OTP Request Status", JOptionPane.INFORMATION_MESSAGE);

            if (serverResponse != null && serverResponse.toLowerCase().startsWith("otp has been sent")) {
                // Enable OTP field and login button only if server indicates OTP was sent
                if (otpField != null) otpField.setEnabled(true);
                if (loginButton != null) loginButton.setEnabled(true);
                if (sendOtpButton != null) sendOtpButton.setEnabled(false); // Disable after successful request
            } else {
                // If server indicates an error (e.g., user not found, email failed), keep OTP field/login disabled
                // and sendOtpButton might remain enabled for retry, or disable based on error type.
                // For simplicity, let's assume sendOtpButton remains enabled if there was an error message from server.
                if (otpField != null) otpField.setEnabled(false);
                if (loginButton != null) loginButton.setEnabled(false);
                if (sendOtpButton != null) sendOtpButton.setEnabled(true); // Allow retry
            }
        } catch (RemoteException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error requesting OTP from server: " + e.getMessage(), "RMI Error", JOptionPane.ERROR_MESSAGE);
            if (sendOtpButton != null) sendOtpButton.setEnabled(true); // Allow retry on RMI error
        }
    }
    ...
    private void signInActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_signInActionPerformed
        ...
        // The check for 'generatedOtp' is removed as client no longer stores it.
        // User is expected to enter OTP received via email.

        if (password.isEmpty() || password.equals("Password")) { // Check for empty or placeholder password
            JOptionPane.showMessageDialog(this, "Password is required.", "Login Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (enteredOtp.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please enter the OTP from your email.", "Login Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            // Call the updated service method that includes password
            Accounts loggedInAccount = accountsService.verifyOtpAndLogin(user, password, enteredOtp);

            if (loggedInAccount != null) {
                // OTP and Password are valid, and not expired
                ...
                // OTP invalid, expired, or user not found by service during verification
                JOptionPane.showMessageDialog(this, "Invalid OTP, or OTP expired, or user not found.", "Login Failed", JOptionPane.ERROR_MESSAGE);
            }
        } catch (RemoteException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error verifying OTP with server: " + e.getMessage(), "RMI Error", JOptionPane.ERROR_MESSAGE);
        } catch (Exception e) { // Catch any other unexpected errors
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "An unexpected error occurred during login: " + e.getMessage(), "Login Error", JOptionPane.ERROR_MESSAGE);
        }
